...
class CreditoTest {
	...
	void setUp() throws Exception {
		...
		TarjetaCredito tarjeta = new TarjetaCredito(1,LocalDate.now().plusYears(1),cliente,1500000);
	...
	@Test
	void testSumaImporteIgualTotal() {
		double total=0;
		for(Detalle detalle : expected.getFactura().getDetalles()) {
			total+=detalle.getImporte();
		}
		assertEquals(total,expected.getFactura().calcularTotal());
	}
	
	@Test
	void testLimiteCredito() {
		assertTrue(expected.getFactura().calcularTotal()<1500000);
	}
	
	@Test
	void testLimiteCreditoYTarjeta() {
		assertTrue(expected.getFactura().calcularTotal()<expected.getTarjetaCredito().getLimiteCompra()&&expected.getFactura().calcularTotal()<1500000);
	}
}
